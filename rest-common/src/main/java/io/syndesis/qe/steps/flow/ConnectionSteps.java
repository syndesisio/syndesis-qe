...
import io.syndesis.qe.utils.aws.SNSUtils;
...
public class ConnectionSteps extends AbstractStep {
    @When("add {string} endpoint with connector id {string} and {string} action and with properties:")
    ...
    @When("create ActiveMQ {string} action step with destination type {string} and destination name {string}")
    ...
    @When("create AMQP {string} action step with properties:")
    ...
    public void createBoxUpload(String fileName) {
        ...
        Map<String, String> properties =
            TestUtils.map("parentFolderId", AccountsDirectory.getInstance().get(Account.Name.BOX).getProperty("folderId"));
        if (!fileName.isEmpty()) {
    ...
    @When("create start DB periodic sql invocation action step with query {string} and period {int} ms")
    public void createStartDbPeriodicSqlStep(String sqlQuery, int ms) {
    ...
    @When("create start DB periodic stored procedure invocation action step named {string} and period {int} ms")
    public void createStartDbPeriodicProcedureStep(String procedureName, int ms) {
        super.addProperty(StepProperty.CONNECTOR_ID, RestTestsUtils.Connector.DB.getId());
        super.addProperty(StepProperty.CONNECTION_ID, RestTestsUtils.Connection.DB.getId());
    ...
    @When("create finish DB invoke sql action step with query {string}")
    ...
    @When("create finish DB invoke stored procedure {string} action step")
    ...
    @When("create Dropbox {string} action step with file path: {string}")
    ...
    @When("create DynamoDB {string} action step with properties:")
    ...
    @When("create Email {string} action with properties:")
    ...
    @When("create FHIR {string} action with resource type {string}")
    ...
    @When("create FTP {string} action with values")
    ...
    private void createHttpStep(String protocol, String method, String path, long period, String timeunit) {
        if ("HTTP".equals(protocol)) {
            ...
            super.addProperty(StepProperty.ACTION, period == -1 ? "http-invoke-url" : "http-periodic-invoke-url");
        } else {
            ...
            super.addProperty(StepProperty.ACTION, period == -1 ? "https-invoke-url" : "https-periodic-invoke-url");
        }
    ...
    @When("create IRC {string} step with nickname {string} and channels {string}")
    ...
    @When("create JIRA {string} step with JQL {string}")
    ...
    @When("create Kafka {string} step with topic {string}")
    ...
    @When("create Kudu {string} step with table {string}")
    ...
    @When("create MongoDB {string} with collection {string}")
    ...
    @When("create OData {string} action with properties")
    ...
    @When("create SF {string} action step with properties")
    ...
    @When("create ServiceNow {string} action with properties")
    ...
    @When("create SFTP {string} action with values")
    ...
    @When("create Slack {string} action with properties")
    ...
    @When("create SNS publish action step with topic {string}")
    ...
    @When("create SQS {string} action step with properties")
    ...
    @When("create S3 polling START action step with bucket: {string}")
    public void createS3PollingStep(String bucketName) {
        createS3PollingStep(bucketName, null);
    }

    @When("create S3 polling START action step with bucket: {string} and properties")
    ...
    @When("create S3 {string} FINISH action step with bucket: {string}")
    public void createS3CopyStep(String action, String bucketName) {
        createS3CopyStepFile(action, bucketName, null);
    }

    @When("create S3 {string} FINISH action step with bucket: {string} and filename: {string}")
    ...
    @When("create TW mention step with {string} action")
