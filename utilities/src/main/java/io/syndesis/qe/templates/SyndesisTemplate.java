...
public class SyndesisTemplate {
    ...
    private static void deployOperator() {
        ...
        final File operator = new File("/tmp/operator");
        if (!TestUtils.isProdBuild()) {
            ...
            // For prod build, we have everything in one file.
            // It is needed to substitute $NAMESPACE$ for correct namespace in the file generated by generate.sh
            try {
                final URL url = TestConfiguration.syndesisOperatorUrl().startsWith("http") ? new URL(TestConfiguration.syndesisOperatorUrl()) : Paths.get(TestConfiguration.syndesisOperatorUrl()).toUri().toURL();
                String content = IOUtils.toString(url, "UTF-8");
                content = content.replaceAll("\\$NAMESPACE\\$", TestConfiguration.openShiftNamespace());
                FileUtils.writeStringToFile(operator, content, "UTF-8");
            } catch (MalformedURLException e) {
                throw new RuntimeException("Unable to create URL from " + TestConfiguration.syndesisOperatorUrl(), e);
            } catch (IOException e) {
                throw new RuntimeException("Unable to read URL " + TestConfiguration.syndesisOperatorUrl(), e);
            }
        }
        OpenShiftBinaryClient.getInstance().executeCommandAndConsumeOutput(
