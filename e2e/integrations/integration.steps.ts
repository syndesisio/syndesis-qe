...
import {
  IntegrationAddStepPage,
  IntegrationEditPage,
  ListActionsComponent,
  StepFactory
} from '../integrations/edit/edit.po';
...
class IntegrationSteps {
  ...
  public editorOpened(): P<any> {
    ...
    const page = new IntegrationEditPage();
    return expect(page.rootElement().isPresent(), 'there must be edit page root element').to.eventually.be.true;
  }
  ...
  public editorOpenedFor(integrationName: string): P<any> {
    return this.editorOpened()
      .then(() => {
        ...
        const page = new IntegrationEditPage();
        return expect(
          page.flowViewComponent().getIntegrationName(),
          `editor must display integration name ${integrationName}`
  ...
  public expectIntegrationPresent(name: string, callback: CallbackStepDefinition): void {
    ...
    const page = new IntegrationsListPage();
    expect(
      page.listComponent().isIntegrationPresent(name),
      `Integration ${name} must be present`
  ...
  public addStepPageOpened(callback: CallbackStepDefinition): void {
    const page = new IntegrationAddStepPage();
    expect(page.rootElement().isPresent(), 'there must be add step page root element').to.eventually.be.true.notify(
      callback
    );
  }
  ...
  public configureStepPageOpen(stepType: string): void {
    ...
    expect(page.rootElement().isPresent(), 'there must be add step page root element').to.eventually.be.true;
    expect(page.validate(), 'page must contain certain elements').to.eventually.be.true;
  }
  ...
  public expectActionListIsPresent(): void {
    ...
    expect(page.rootElement().isDisplayed(), 'There must be action list loaded').to.eventually.be.true;
  }
}
