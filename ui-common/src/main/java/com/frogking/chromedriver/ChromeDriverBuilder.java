    ...
    private void handlePrefs(String userDataDir, Map<String, Object> prefs) throws RuntimeException {
        ...
        if (new File(prefsFile).exists()) {
            ...
            try {
                ...
                newPrefs = new JSONObject(stringBuilder.toString()).toMap();
            } catch (Exception e) {
            ...
            try (BufferedWriter bw = new BufferedWriter(new FileWriter(prefsFile))) {
                bw.write(new JSONObject(newPrefs).toString());
                bw.flush();
            } catch (Exception e) {
