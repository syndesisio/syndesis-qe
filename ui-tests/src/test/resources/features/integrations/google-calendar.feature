...
Feature: Google Calendar Connector
  ...
  Scenario: Test Copy Event
    ...
    And following "all" events in calendar "syndesis-test2" with account "QE Google Calendar"
    ...
    # write to syndesis-test1 calendar
    When select the "My Google Calendar Connector" connection
      ...
      And click on the "Done" button
      # prepend a get event from syndesis-test2 calendar
    ...
    When defined constant "new_event" of type "String" in data mapper
      And defined constant "about_the_event" of type "String" in data mapper
  ...
  Scenario: Test Get a specific Event
    Given navigate to the "Home" page
      And following "all" events in calendar "syndesis-test1" with account "QE Google Calendar"
  ...
  Scenario: Test Update event
    ...
    When select the "My Google Calendar Connector" connection
      ...
      And fill in update event form using calendar "syndesis-test1" summary "updated_summary" and description "This event has ended. Recording: xy" for user "QE Google Calendar"
      And click on the "Done" button
      And add integration "connection" on position "0"
      And select the "My Google Calendar Connector" connection
      And select "Get a specific Event" integration action
      ...
      And select "Data Mapper" integration step
    Then check visibility of data mapper ui
    When defined constant "updated_summary" of type "String" in data mapper
      And defined constant "this_event_has_ended" of type "String" in data mapper
      And open data bucket "Constants"
      And create data mapper mappings
        | updated_summary      | title       |
        | this_event_has_ended | description |
        | startDate            | startDate   |
        | endDate              | endDate     |
        | attendees            | attendees   |
        | location             | location    |
  ...
  Scenario Outline: Test get events action - <name> on <events_to_create> events
    Given navigate to the "Home" page
      And following "<events_to_create>" events in calendar "syndesis-test1" with account "QE Google Calendar"
    ...
    Examples:
      ...
      | poll_updated  | future           | 5           | false                   | true                   |                  | some                  | 2 rows                |
      | poll_upcoming | future           | 5           | true                    | false                  |                  | some                  | 2 rows                |
      | poll_all      | all              | 5           | false                   | false                  |                  | some                  | 4 rows                |
      | poll_updated  | all              | 5           | false                   | true                   |                  | some                  | 2 rows                |
  ...
  Scenario: Test get events action - poll events updated during runtime
    ...
    And updated event "past_event1" in calendar "syndesis-test1" for user "QE Google Calendar" with values
