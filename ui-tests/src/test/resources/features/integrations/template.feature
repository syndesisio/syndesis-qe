...
Feature: Templates
  ...
  Scenario: Verify that the code editor for templates is working correctly
    ...
    And upload template from resource "templates/code_editor_test.tpl"
    ...
    #TODO: change this to use correct line number after #5273 gets resolved
    And check that there is an error message with text "'{{wrongid1}}' does not conform to the format {{xyz}} at line: 2, column: 12" in the template editor
